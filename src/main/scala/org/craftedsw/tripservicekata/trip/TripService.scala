...
class TripService(getLoggedUser: () => Option[User], findTripsByUser: User => List[Trip] ) {

	def this(getLoggedUser: () => User) = this(() => Option(getLoggedUser()), _ => Nil)

	def this() =  this( () => Option(UserSession.getLoggedUser), TripDAO.findTripsByUser )
